...
function BuscarObjetivo({ d }: { d: Draft }) {
  const escala = useTypeScale();
...
function Direcciones({ d }: { d: Draft }) {
  const escala = useTypeScale();
  const [texto, setTexto] = useState("");
...
const celdaDeAlta = (d: Draft, cerrar: () => void): WidgetDefinition[] => [
  {
    id: "policies/nueva",
    label: "New policy",
    icon: ShieldCheck,
    crudo: true,
    glance: () => <FichaDePolitica d={d} cerrar={cerrar} />,
    full: () => <FichaDePolitica d={d} cerrar={cerrar} />,
  },
];
...
export function useAltaDePolitica(tabId?: string) {
  ...
  useEffect(() => {
    ...
    mostrarWidgets(tabId, celdaDeAlta(d, cerrar));
    abrirBoard(tabId);
  }, [tabId, abierta, d, cerrar, mostrarWidgets, abrirBoard]);
